 ...
 */
export async function generateAudio(_prompt: string, _voiceId = "alloy"): Promise<string> {
 ...
 */
export async function transcribeAudio(_base64Audio: string): Promise<string> {
    const apiKey = process.env.NVIDIA_API_KEY;
    if (!apiKey) {
        throw new Error("NVIDIA_API_KEY is not configured in Convex environment variables.");
    }
